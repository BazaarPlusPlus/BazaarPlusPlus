...
mod 与 installer 是同一个 Product Release 的两个产物。根目录 `VERSION` 是唯一手工维护的产品版本；`node release.mjs sync` 将它投影到 npm、Tauri、Cargo 和 README，MSBuild 直接读取它。数据库 schema、native ABI、V5 用户数据格式以及用户当前安装版本保持独立。

## 入口

在仓库根目录执行，Windows 使用 Git Bash；每个平台在自己的原生构建机上准备和打包。

```bash
node release.mjs sync
node release.mjs check
node release.mjs prepare --platform macos
node release.mjs build --platform macos
node release.mjs upload --platform macos
node release.mjs promote
```

Windows 将 `macos` 换成 `windows`。`prepare` 和 `build` 可通过 `-- -p:ManagedPath=<absolute-path>` 指定正式服游戏程序集；不接受编译器、版本、目标或输出目录覆盖。`build` 包含 `prepare`，但不会自动上传；`upload` 不修改 latest；只有 `promote` 发布完整双平台版本。mod 的 `./run.sh publish`、installer 的 `./build.sh --prod` 和 `npm run prepare:resources -- --platform …` 均转入这个入口。

日常开发仍在子目录进行。mod 的 `./run.sh build --no-deploy` 只编译，不修复 trampoline，也不修改游戏安装。
...
- 根目录：`node release.mjs check`。
- installer：`npm run verify -- --source-only` 和 `npm run docs:check`；真实准备后使用 `npm run verify -- --release-platform macos`（或 `windows`）。跨仓库发布测试还需要 .NET SDK。
- mod：`./run.sh build --no-deploy`、`./run.sh test`。
- site：`npm test`、`npm run typecheck`、`npm run lint`、`npm run format:check`、`npm run build`。
